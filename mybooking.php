                                                        ...
                                                        if (new DateTime() <= new DateTime($bdate)){
                                                            ...
                                                            if ($row = mysqli_fetch_assoc($viewresult)) {
                                                                echo "<script>alert(\"Sorry. This train unavailable at this Day\");</script>";
                                                                    ...
                                                                    if ($trainclass == 1) {
                                                                         $total = $traveller * $row1['one_class_price'];
                                                                    }elseif ($trainclass == 2) {
                                                                        $total = $traveller * $row1['two_class_price'];
                                                                    }elseif ($trainclass == 3) {
                                                                        $total = $traveller * $row1['tree_class_price'];
                                                                    }


                                                                    $q1="INSERT INTO  booking(sch_id,booking_date,traveller,total_amount,payment,status,trndate) values('$trainsch','$bdate','$traveller','$total','Pending','Pending','$cdate')";
                                                                      $res1=mysqli_query($con,$q1);

                                                                      if ( $res1)
                                                                      {
                                                                          echo '<script>alert("Booking Saved is Scussessfully."); window.location.href="booklist.php";
                                                                            </script>'; 
                                                                      }else{
